...
struct mocked_queue {
    ...
    using queued_value_t = queued_value<value_type, mocked_executor>;

    MOCK_CONST_METHOD3(push, bool (const mocked_executor&, time_traits::duration, const value_type&));
...
using resource_pool_impl = pool_impl<resource, std::mutex, mocked_executor, mocked_queue>;
using resource_ptr_list_iterator = resource_pool_impl::list_iterator;
using queued_value_t = mocked_queue::queued_value_t;

auto make_queued_value(mocked_queue::value_type&& request, const mocked_executor& ex) {
    return boost::optional<queued_value_t>(queued_value_t {std::move(request), ex});
}
...
struct async_resource_pool_impl : Test {
    ...
    // Everything the pool hands to the executor, in order.
    //
    // These tests used to name each submission individually, which worked while
    // Boost.Asio had separate post and dispatch entry points to tell them apart.
    // It has one execute() now, and the library does not submit the same number
    // of times it did in 2021, so a test that captures exactly two handlers
    // silently drops the third and then fails on an unrelated assertion. Draining
    // the queue instead asserts what the tests actually mean: run the work the
    // pool scheduled, then check the resulting state.
    std::deque<std::function<void ()>> submitted;

    void collect_submissions() {
        EXPECT_CALL(executor, execute(_))
            .Times(AnyNumber())
            .WillRepeatedly(Invoke([this] (std::function<void ()> f) {
                submitted.push_back(std::move(f));
            }));
    }

    // Runs scheduled work until none is left, including anything scheduled by
    // the handlers themselves. Bounded so that a pool which reschedules forever
    // fails the test rather than hanging it.
    void run_submitted(std::size_t limit = 64) {
        for (std::size_t n = 0; !submitted.empty(); ++n) {
            ASSERT_LT(n, limit) << "scheduled work did not settle";
            auto next = std::move(submitted.front());
            submitted.pop_front();
            next();
        }
    }
...
TEST_F(async_resource_pool_impl, get_one_should_call_callback) {
    ...
    EXPECT_CALL(executor, execute(_)).WillOnce(SaveArg<0>(&on_get));
    EXPECT_CALL(*get, call(_, _)).WillOnce(Return());

    pool.get(executor_wrapper, callback(get));
    on_get();
}
...
TEST_F(async_resource_pool_impl, get_one_and_recycle_should_make_one_available_resource) {
    ...
    pool.get(executor_wrapper, recycle_resource(pool));
    on_get();
...
TEST_F(async_resource_pool_impl, get_one_and_waste_should_make_no_available_resources) {
    ...
    pool.get(executor_wrapper, waste_resource(pool));
    on_get();
...
TEST_F(async_resource_pool_impl, get_twice_and_recycle_should_make_one_available_resource) {
    ...
    EXPECT_CALL(executor, execute(_)).WillOnce(SaveArg<0>(&on_first_get));
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(boost::none)));
    pool.get(executor_wrapper, recycle_resource(pool));
    on_first_get();

    EXPECT_CALL(executor, execute(_)).WillOnce(SaveArg<0>(&on_second_get));
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(boost::none)));
    pool.get(executor_wrapper, recycle_resource(pool), time_traits::duration(1));
    on_second_get();
...
TEST_F(async_resource_pool_impl, get_twice_and_recycle_should_use_queue_and_make_one_available_resource) {
    ...
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(make_queued_value(std::move(on_get_res), executor_wrapper))));
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(boost::none)));
    run_submitted();
...
TEST_F(async_resource_pool_impl, get_twice_and_waste_then_get_should_use_queue) {
    ...
    EXPECT_CALL(pool.queue(), push(_, _, _)).WillOnce(DoAll(SaveMoveArg2(&on_get_res), Return(true)));
    pool.get(executor_wrapper, waste_resource(pool));
    pool.get(executor_wrapper, waste_resource(pool), time_traits::duration(1));
...
TEST_F(async_resource_pool_impl, get_with_queue_zero_capacity_use_should_return_error) {
    ...
    pool.get(executor_wrapper, recycle_resource(pool));
    pool.get(executor_wrapper, check_error(error::request_queue_overflow), time_traits::duration(1));
    run_submitted();
...
TEST_F(async_resource_pool_impl, get_with_queue_use_and_timer_timeout_should_return_error) {
    ...
    pool.get(executor_wrapper, check_no_error());
    pool.get(executor_wrapper, check_error(error::get_resource_timeout), time_traits::duration(1));
    on_first_get();
...
TEST_F(async_resource_pool_impl, get_with_queue_use_with_zero_wait_duration_should_return_error) {
    ...
    pool.get(executor_wrapper, recycle_resource(pool));
    pool.get(executor_wrapper, check_error(error::get_resource_timeout), time_traits::duration(0));
    run_submitted();
...
TEST_F(async_resource_pool_impl, get_after_disable_returns_error) {
    ...
    pool.disable();
    pool.get(executor_wrapper, check_error(error::disabled));
    on_get();
}
...
TEST_F(async_resource_pool_impl, get_recycled_after_disable_returns_error) {
    ...
    EXPECT_CALL(pool.queue(), push(_, _, _)).WillOnce(DoAll(SaveMoveArg2(&on_get_res), Return(true)));
    pool.get(executor_wrapper, recycle_resource(pool));
    pool.get(executor_wrapper, check_error(error::disabled), time_traits::duration(1));
...
TEST_F(async_resource_pool_impl, get_one_set_and_recycle_with_zero_idle_timeout_then_get_should_return_empty) {
    ...
    EXPECT_CALL(executor, execute(_)).WillOnce(SaveArg<0>(&on_first_get));
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(boost::none)));
    pool.get(executor_wrapper, set_and_recycle_resource(pool));
    on_first_get();

    EXPECT_CALL(executor, execute(_)).WillOnce(SaveArg<0>(&on_second_get));
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(boost::none)));
    pool.get(executor_wrapper, assert_empty(pool), time_traits::duration(1));
    on_second_get();
}
...
TEST_F(async_resource_pool_impl, should_restore_wasted_cell) {
    ...
    EXPECT_CALL(executor, execute(_)).WillOnce(SaveArg<0>(&on_second_get));
    EXPECT_CALL(pool.queue(), pop()).WillOnce(Return(ByMove(boost::none)));
    pool.get(executor_wrapper, set_and_recycle_resource(pool), time_traits::duration(1));
    on_second_get();
